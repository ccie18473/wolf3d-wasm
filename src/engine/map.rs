...
impl Map {
    fn parse_cells_from_json(json: &serde_json::Value) -> Cells {
        ...
        for list in json.as_array().unwrap() {
            let mut floor: Vec<Vec<Cell>> = (0..100)
                .map(|_| (0..100).map(|_| Cell::default()).collect())
                .collect();
    ...
    fn parse_portals_from_json(json: &serde_json::Value) -> Portals {
        ...
        if let Some(list) = json.as_array() {
            for portal in list {
                portals.push((
                    Portal::from_json(&portal["first"]),
                    Portal::from_json(&portal["second"]),
                ));
            }
        }
        portals
    ...
    pub fn portals_at(
        ...
        position: Vector3f,
        dir: Direction,
    ) -> Option<(Option<&Portal>, Option<&Portal>)> {
