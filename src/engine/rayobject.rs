...
impl Ray {
    pub fn new(player: &Player, camera_dir: Vector2f) -> Ray {
        ...
        let dir = Vector3f::new(
            player.dir.x + player.plane.x * camera_dir.x,
            player.dir.y + player.plane.y * camera_dir.x,
            0.5 * camera_dir.y,
        );
        ...
        side_dist.x = if dir.x < 0.0 {
            origin.x - pos.x
        } else {
            pos.x + 1.0 - origin.x
        } * delta.x;
        side_dist.y = if dir.y < 0.0 {
            origin.y - pos.y
        } else {
            pos.y + 1.0 - origin.y
        } * delta.y;
        side_dist.z = if dir.z < 0.0 {
            pos.z - origin.z + 0.5
        } else {
            origin.z - pos.z + 0.5
        } * delta.z;
    ...
    fn relocate(&mut self, new_origin: Vector3f, new_dir: f32) {
        ...
        self.delta = Vector3f::new(
            (1.0 / self.ray_dir.x).abs(),
            (1.0 / self.ray_dir.y).abs(),
            (1.0 / self.ray_dir.z).abs(),
        );
        self.side_dist.x = if self.ray_dir.x < 0.0 {
            self.origin.x - self.pos.x
        } else {
            self.pos.x + 1.0 - self.origin.x
        } * self.delta.x;
        self.side_dist.y = if self.ray_dir.y < 0.0 {
            self.origin.y - self.pos.y
        } else {
            self.pos.y + 1.0 - self.origin.y
        } * self.delta.y;
        self.side_dist.z = if self.ray_dir.z < 0.0 {
            self.pos.z - self.origin.z + 0.5
        } else {
            self.origin.z - self.pos.z + 0.5
        } * self.delta.z;
    ...
    pub fn cast(&mut self, map: &Map) -> Hit {
        ...
        let mut passed_door_pos = Vector3f::default();
        ...
        loop {
            if passed_through {
                ...
                if (self.dir == Direction::Up
                    || (self.dir.is_side() && texture_pos.y <= passed_height))
                    && self.is_block_adjacent(passed_pos, self.pos)
                {
                    let dir = if self.ray_dir.z > 0.0 {
                        Direction::Up
                    } else {
                        Direction::Down
                    };
                    let dist = (passed_pos.z - (1.0 - passed_height) - self.origin.z
                        + (1.0 - self.step.z) / 2.0)
                        / self.ray_dir.z;
                    let texture_pos = self.compute_texture_pos(&dir, dist);
                    return Hit {
                        value: Some(map.get(&passed_pos).value()),
                        pos: self.pos,
                        dist,
                        dir,
                        texture_pos,
            ...
            if passed_door
                && (!self.is_block_adjacent(passed_door_pos, self.pos)
                    || self.pos.z != passed_door_pos.z)
            {
                passed_door = false;
            }
            ...
            match map.get(&self.pos) {
                Cell::Empty => {
                    if self.pos.z < 0.0
                        || self.pos.z >= map.depth() as f32
                        || ((self.pos.x - self.origin.x).powf(2.0)
                            + (self.pos.y - self.origin.y).powf(2.0))
                        .sqrt()
                            > max_dist
                    {
                        let dist = self.compute_dist();
    ...
    fn grow_x(&mut self) {
        ...
        self.dir = if self.origin.x < self.pos.x {
            Direction::East
        } else {
            Direction::West
        };
    ...
    fn grow_y(&mut self) {
        ...
        self.dir = if self.origin.y < self.pos.y {
            Direction::North
        } else {
            Direction::South
        };
    ...
    fn grow_z(&mut self) {
        ...
        self.dir = if self.ray_dir.z > 0.0 {
            Direction::Up
        } else {
            Direction::Down
        };
    ...
    fn grow_thin(&mut self, cell: &Box<dyn Thin>) -> Option<(f32, Vector2f)> {
        ...
        let limit_x = if dir == Direction::East {
            depth
        } else {
            1.0 - depth
        };
        let limit_y = if dir == Direction::North {
            depth
        } else {
            1.0 - depth
        };
        ...
        let ray2 = Vector2f::new(
            self.origin.x + self.ray_dir.x * ray_mult,
            self.origin.y + self.ray_dir.y * ray_mult,
        );
        let delta = Vector2f::new(
            (1.0 + self.ray_dir.y.powf(2.0) / self.ray_dir.x.powf(2.0)).sqrt(),
            (1.0 + self.ray_dir.x.powf(2.0) / self.ray_dir.y.powf(2.0)).sqrt(),
        );
        let true_step = Vector2f::new(
            (delta.y.powf(2.0) - 1.0).sqrt(),
            (delta.x.powf(2.0) - 1.0).sqrt(),
        );
        let half_step_in = Vector2f::new(
            ray2.x + (self.step.x * true_step.x) * depth,
            ray2.y + (self.step.y * true_step.y) * depth,
        );
        ...
        if !under_light {
            if half_step_in.y.floor() == self.pos.y && (self.pos.y - half_step_in.y).abs() <= slide
            {
                ...
                let mut texture_pos = Vector2f::new(
                    self.origin.y + dist * self.ray_dir.y - slide,
                    self.origin.z + dist * -self.ray_dir.z,
                );
            ...
            if half_step_in.x.floor() == self.pos.x && (self.pos.x - half_step_in.x).abs() <= slide
            {
                ...
                let mut texture_pos = Vector2f::new(
                    self.origin.x + dist * self.ray_dir.x - slide,
                    self.origin.z + dist * -self.ray_dir.z,
                );
                if texture_pos.y < self.pos.z || texture_pos.y > self.pos.z + 1.0 {
                    return None;
    ...
    fn compute_texture_pos(&self, dir: &Direction, dist: f32) -> Vector2f {
        let mut texture_pos = match dir {
            ...
            Direction::North | Direction::South => Vector2f::new(
                self.origin.x + dist * self.ray_dir.x,
                self.origin.z + dist * -self.ray_dir.z,
            ),
            Direction::West | Direction::East => Vector2f::new(
                self.origin.y + dist * self.ray_dir.y,
                self.origin.z + dist * -self.ray_dir.z,
            ),
            Direction::Up | Direction::Down => Vector2f::new(
                ((self.pos.x - self.origin.x + (1.0 - self.step.x) / 2.0) / self.ray_dir.x + dist)
                    * self.ray_dir.x,
                ((self.pos.y - self.origin.y + (1.0 - self.step.y) / 2.0) / self.ray_dir.y + dist)
                    * self.ray_dir.y,
            ),
        };
        texture_pos.x -= texture_pos.x.floor();
