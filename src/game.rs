...
pub struct Game {
    ...
    pub canvas: Canvas,
    pub z_buffer: Vec<Vec<Zdist>>,
    pub z_origins: Vec<Zorigin>,
}

impl Game {
    pub fn new(ctx: &mut Context, map: &serde_json::Value, width: usize, height: usize) -> Game {
        Game {
            map: Map::new(map),
    ...
    pub fn new_size(&mut self, ctx: &mut Context, width: usize, height: usize) {
        self.canvas = Canvas::new(ctx, width, height);
        self.z_buffer = vec![vec![Zdist::default(); width]; height];
        self.z_origins = Vec::new();
    }
    fn compute_pixel(&mut self, x: usize, y: usize, hit: &Hit, ray: &mut Ray) -> RGBColor {
        self.z_buffer[y][x] = Zdist {
            dist: hit.dist.abs(),
            portal_depth: ray.portal_recursion,
        };
        match hit.value {
            None => {
            ...
            Some(value) => {
                let (text_x, text_y) =
                    textures::get_texture_coord(hit.texture_pos.x, hit.texture_pos.y);

                let mut color = match self.map.portals_at(hit.pos, hit.dir) {
                    None => textures::get_wall_pixel(
                        text_x + 64 * !hit.dir.is_under_light() as u32,
                        text_y + 64 * value,
                    ),
                    Some((source, dest)) => {
                        ...
                        if source.is_some() && dest.is_some() {
                            ...
                            if portal_color == WHITE {
                                ...
                                //Portal's border
                                RGBColor::from_hsl(
                                    &(HSLColor::from_rgb(&portal_color) + source.hsl),
                                )
                            } else {
                                //Portal's center
                                if ray.pass_through_portal(dest, source) {
                                    ...
                                    let mut find = false;
                                    for idx in (0..self.z_origins.len()).rev() {
                                        if self.z_origins[idx].pos == pos
                                            && self.z_origins[idx].depth == depth
                                        {
                                            find = true;
                                            break;
                                        }
                            ...
                            if portal_color == WHITE {
                                ...
                                RGBColor::from_hsl(
                                    &(HSLColor::from_rgb(&portal_color) + source.unwrap().hsl),
                                )
                            }
                        }
                    }
        ...
        for x in 0..self.canvas.width {
            for y in 0..self.canvas.height {
                let mut ray = Ray::new(
                    &self.player,
                    Vector2f::new(
                        2.0 * x as f32 / canvas_width - 1.0,
                        2.0 * y as f32 / canvas_height - 1.0,
                    ),
                );
    ...
    fn update_sprite(
        ...
        for sprite in &self.sprites {
            ...
            let transform = Vector2f::new(
                (dir.y * relative_pos.x - dir.x * relative_pos.y) * 1.0
                    / (plane.x * dir.y - dir.x * plane.y),
                (-plane.y * relative_pos.x + plane.x * relative_pos.y) * 1.0
                    / (plane.x * dir.y - dir.x * plane.y),
            );

            let sprite_canvas_x =
                ((self.canvas.width / 2) as f32 * (1.0 + transform.x / transform.y)) as i32;
            let sprite_size = (canvas_height as f32 / transform.y).abs() as i32;

            let x_bounds = Vector2i::new(
                sprite_canvas_x - sprite_size / 2,
                sprite_size / 2 + sprite_canvas_x,
            )
            .clamp(0, canvas_width, 0, canvas_width);
            let draw_end_y = ((self.canvas.height / 2) as f32 + sprite_size as f32 * (pos.z + 0.5)
                - sprite_size as f32 * sprite.pos.z) as i32;
            let y_bounds = Vector2i::new(draw_end_y - sprite_size, draw_end_y).clamp(
                0,
                canvas_height,
                0,
                canvas_height,
            ...
            for x in x_bounds.x..x_bounds.y {
                ...
                if transform.y > 0.0 && x > 0 && x < canvas_width {
                    ...
                    let mut text_pos = ((y_bounds.x as f32
                        - sprite_size as f32 * (pos.z - sprite.pos.z))
                    ...
                    for y in y_bounds.x..y_bounds.y {
                        ...
                        text_pos += step;
                        if (depth != self.z_buffer[y as usize][x as usize].portal_depth
                            && !sprite.is_player)
                            || transform.y > self.z_buffer[y as usize][x as usize].dist
                        {
                            continue;
                        }
                        let color = if sprite.is_player {
                            let mut rotation_degree =
                                (rotation * 180.0 / std::f32::consts::PI).round() % 360.0;
                            let mut player_degree =
                                ((self.player.dir.y / self.player.dir.x).atan() * 180.0
                                    / std::f32::consts::PI)
                                    .round();
                            ...
                            if (self.player.dir.x > 0.0 && self.player.dir.y < 0.0)
                                || (self.player.dir.x > 0.0 && self.player.dir.y > 0.0)
                            {
                            ...
                            let mut value = -(4
                                + ((rotation_degree / 45.0).round()
                                    - ((player_degree - shift_degree) / 45.0).round())
                                    as i32)
                                % 8;
                            if value < 0 {
                                value = 8 + value;
                            }
                            textures::get_soldier_pixel(
                                text_x + 64 * value as u32,
                                text_y + 64 * self.player.frame,
                            )
                        } else {
